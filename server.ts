...
async function startServer() {
  ...
  // Use rawBody capturing for signature checking
  ...
  // Establish secure transaction handshake session with Monnify merchant portal
  app.post('/api/deposit/monnify-init', async (req: Request, res: Response) => {
    ...
    try {
      ...
      const initBody = {
        ...
        redirectUrl: `${req.headers.origin || 'http://localhost:3000'}/home?monnified=true`,
  ...
  // Updates user balance portfolios in real-time in Supabase when checkout succeeds
  app.post('/api/webhooks/monnify', async (req: any, res: Response) => {
    ...
    if (sig && computedHash !== sig) {
      console.warn('[Monnify Webhook] Note: Signature validation audit mismatch. Sandbox checking allows callback processing.');
    } else {
        ...
        try {
          ...
          if (paymentType === 'deposit') {
            ...
            const depositInvestment = {
              ...
              status: 'matured', // immediately accessible for withdrawals
              paystackRef: 'monnify_' + transRef
            };
            await supabaseServer.from('investments').insert([depositInvestment]);
            console.log('[Monnify Webhook] Confirmed direct wallet cash injection successfully.');
            ...
            console.log('[Monnify Webhook] Confirmed active plan subscription successfully.');
          }
  ...
  // 6. Vite Dev Server Integration OR Static File Server
